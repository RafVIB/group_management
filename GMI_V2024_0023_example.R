...
survival <- 0.82        # overarching survival rate
sex_ratio_N <- 0.5      # sex ratio of the founders
age_max <- 14           # maximum longevity of the species
first_repro <- 3        # age at which an individual of this species reproduces for the first time
last_repro <- 12        # age at which an individual of this species reproduces for the first time
species_type <- 'K'     # specify whether the species is an r species ('r') or K species ('K') (couples are defined in a different way)

mating_system <- "M"    # "M": monogamous; "P": polygynous; "Ha": harem
harem_size <- 1         # number of females per male
stdev_harem_size <- 0   # standard deviation harem size
litter_size <- 3.66     # average litter size
sd_litter_size <- 1.57  # standard deviation litter size

# run parameters

generations <- 100     # number of generations you want to run the simulation
simulations <- 5       # number of simulations you want to run

# population structure parameters population

NG_BC <- 4                    # number of groups in a breeding circle
NBC_I <- 2                    # number of breeding circles per institute
NI <- 1                       # number of institutes

NG<-NG_BC*NBC_I*NI            # total number of groups

holding_populations <- 1      # 1 =  present; 0 = absent
restocking <- 1               # 1 =  yes; 0 =  no
n_restock <- 5                # number of individuals taken from holding pop to restock crashed pops
years_in_holding <- 5         # maximum number of years an individual can stay in a holding population, before it is removed from the system
level_holding_pop <- "I"      # holding populations can be absent or installed at the breeding circle, institute or global level: "non","BC","I","global"
K_HP <- 50                    # maximum size ("carrying capacity") of a holding population

# molecular data parameters
mol_data <- 'I'      # 'I': random data each founder unique; 'II' : real data manual input
no_markers <- 10
store_history <- 0   # 1 = keep track of all historic individuals; 0 = only keep track of living individuals

#transfer parameters
transfer_age <- c(1)    # list the ages at which animals may be transferred (e.g., if c(1), only individuals with an age of one year will be transferred)
transfer_gender <- c(1) # list the sexes that may be transferred (e.g., if c(1), only males will be transferred)

kinships <- 1        # 1 if you want to calculate a kinship matrix
kin_method <- 2      # method of calculating kinship: 1 = pedigree based; 2 = based on molecular data

# define the combination of metapopulation designs and transfer strategies to be tested
GroupSize <- seq(30,50,10)  # specify the minimum and maximum group size to be simulated, and the step
Transfer_Strategy <- c("LINE_BREEDING","RANDOM","CB","FALCONERS","MAI","MK")  # specify the breeding strategies you want to simulate (options: "LINE_BREEDING": line breeding;"RANDOM": random breeding;"CB": circular breeding; "FALCONERS": falconers' breeding scheme; "MAI": maximum avoidance of inbreeding;"MK": mean kinship breeding)
TFG <- seq(1,21,5)    # specify transfer frequency between groups
TFHL <- seq(2,10,4)   # specify the transfer frequency ratio at the higher level
FounderSize <- c(10,20)   # specify the founder sizes (= number of individuals in each group at the start) you want to simulate
